...
POC中只有在`liveRankingVisible=true`且Race处于`open`状态时，ARY才读取投影正文。关闭披露后，ARY停止展示并清空当前内存投影和版本校验基线；重新公开时，ARY读取当前投影文件并把文件内`version`作为新的有效基线，版本号仍由Organizer侧生成。
...
实时披露数据由Organizer侧生成，例如`live-ranking.json`。Organizer可以实时修改、关闭或删除投影。ARY只能展示当前可用投影。披露关闭期间，ARY不读取投影正文；重新开启披露时，ARY以当前文件内容重新建立展示基线。
...
| 实时排名正文 | Organizer | 不长期保存 | 内存缓存 | 关闭投影后清空内存投影，重新开启时从当前投影文件重建基线 |
...
| Projection Layer | JSON文件+文件监听+轻量轮询扫描 | 监听并兜底扫描`live-ranking.json`变化 |
...
- ARY没有完整评分系统。
- ARY没有完整排名数据。
- Organizer关闭投影后，ARY停止展示并清空当前内存投影；强制结束Race时才冻结已公开版本。
...
| Organizer可关闭投影 | 披露开关和投影状态 | 关闭后ARY停止展示、清空内存投影和版本基线；重新开启后读取当前投影作为新基线 | E2E测试 |
...
- `version`必须是正整数。
- 披露开启且已有有效基线时，新投影`version`必须严格大于上一有效版本；相同版本内容变化视为冲突并拒绝。
- 披露关闭后，ARY清空当前有效基线；重新开启披露时，当前投影文件的`version`会被接受为新的基线，不由ARY自动递增或重置。
- `updatedAt`必须是合法时间。
- `scores`必须是数组。
...
- 仅在Race处于`open`且披露开关开启时读取投影文件。
- 使用`fs.watch`监听文件变化，并通过轻量轮询扫描`mtime/size/hash`变化兜底，避免原子替换或跨目录移动时遗漏更新。
- 校验JSON Schema。
- 计算SHA256。
- 按`score`降序排序。
- 内存缓存排名正文。
- 持久化版本、哈希、同步时间、状态。
- 通过SSE推送给前端。
- 披露关闭时清空内存排名和版本校验基线，只保留不可用的同步元信息；关闭期间不读取`scores`正文。
...
sequenceDiagram
  ...
  ARY->>ARY: 清空内存排名和版本校验基线
  ARY->>Frontend: SSE推送空排名或隐藏排名
  Organizer->>Organizer: 关闭期间可继续修改live-ranking.json
  Organizer->>ARY: 再次POST disclosure开启liveRankingVisible
  ARY->>ARY: 读取当前live-ranking.json并采用文件内version作为新基线
  ARY->>Frontend: SSE推送重新公开后的排名
```
...
| 投影关闭 | Organizer关闭披露后ARY停止展示并清空内存投影；关闭期间修改文件不会产生同步异常，重新公开后以当前文件重建基线 |

...
function render() {
  $("raceList").innerHTML = state.races.length ? state.races.map(r => `<button class="race-select ${r.raceId === selectedRaceId ? "secondary" : ""}" data-race-id="${r.raceId}">${escapeHtml(r.manifest.disclosure.title)} · ${statusLabel(r.status)} · ${r.datasetUploaded ? "赛题已上传" : "待上传赛题"}</button>`).join("") : `<div class="empty">尚未创建赛事</div>`;
  const r = selected(); $("detail").classList.toggle("hidden", !r); if (!r) return; const m = r.manifest.disclosure;
  $("detailTitle").textContent = m.title; $("titleInput").value = m.title; $("summaryInput").value = m.summary; $("posterVisible").checked = m.posterVisible; $("downloadPermission").checked = r.downloadPolicy.enabled;
  ...
  $("datasetStatus").textContent = r.datasetUploaded ? "专属赛题已上传，可以下载。" : "尚未上传赛题，Racer 可以参加和提交，但不能下载。";
...
async function refresh(id = selectedRaceId) { state = await api("/api/state"); selectedRaceId = state.races.some(r => r.raceId === id) ? id : state.races[0]?.raceId; render(); }
$("createRace").onclick = async () => { const x = await api("/api/organizer/races", { method: "POST", body: JSON.stringify({ title: $("createTitle").value, summary: $("createSummary").value, startsAt: new Date($("createStartsAt").value).toISOString(), endsAt: new Date($("createEndsAt").value).toISOString() }) }); toast("赛事已创建"); await refresh(x.race.raceId); };
$("raceList").onclick = e => { const b = e.target.closest("[data-race-id]"); if (b) { selectedRaceId = b.dataset.raceId; render(); } };
$("saveDisclosure").onclick = async () => { await api(`/api/organizer/races/${selectedRaceId}/disclosure`, { method: "POST", body: JSON.stringify({ title: $("titleInput").value, summary: $("summaryInput").value, posterVisible: $("posterVisible").checked }) }); toast("披露已更新"); await refresh(); };
...
$("uploadDataset").onclick = async () => { const x = await uploadPdf(`/api/organizer/races/${selectedRaceId}/dataset`, "datasetFile"); $("datasetResult").textContent = `大小：${x.size} B\nSHA256：${x.sha256}`; toast("赛题已上传"); await refresh(); };
$("saveDownloadPermission").onclick = async () => { await api(`/api/organizer/races/${selectedRaceId}/download-permission`, { method: "POST", body: JSON.stringify({ enabled: $("downloadPermission").checked }) }); toast("权限已更新"); await refresh(); };

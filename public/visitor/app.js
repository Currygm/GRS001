...
function show(a) {
  ...
  $("archiveDetail").innerHTML = `<span class="eyebrow">${a.raceId} · v${a.version}</span><h2>${escapeHtml(a.title)}</h2><p>${escapeHtml(a.summary)}</p><object class="pdf-view" data="${a.posterUrl}" type="application/pdf"><a class="download-link" href="${a.posterUrl}" target="_blank">打开最终 PDF 海报</a></object><h3>最终排名</h3>${a.results.map(r => `<div class="audit"><span>第 ${r.rank} 名</span><strong>${escapeHtml(r.racerId)}</strong><div>${escapeHtml(String(r.score))}</div></div>`).join("")}<h3>优秀作品</h3>${a.showcases.length ? a.showcases.map(s => `<div class="panel"><span class="eyebrow">第 ${s.rank} 名 · ${escapeHtml(s.racerId)} · ${escapeHtml(String(s.score))}</span><strong>${escapeHtml(s.title)}</strong><p>${escapeHtml(s.summary)}</p>${s.demoUrl ? `<a class="download-link" href="${escapeHtml(s.demoUrl)}" target="_blank">访问公开 Demo</a>` : ""}</div>`).join("") : `<div class="empty">暂无优秀作品摘要</div>`}<p>${formatDateTime(a.startsAt)} 至 ${formatDateTime(a.endsAt)} · 发布于 ${formatDateTime(a.publishedAt)}</p>`;
}
